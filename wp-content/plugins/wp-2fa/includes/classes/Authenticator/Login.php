...
class Login {
	...
	public static function wp_login( $user_login, $user ) {
		...
		// exit;	/*custom code - comment out exit to continue action execution */
	}
	...
	public static function login_html( $user, $login_nonce, $redirect_to, $error_msg = '', $provider = null ) {
		...
		<?php
		/*custom code - Prepare custom form HTML and store it in a transient to call on the page template later */

		$custom_auth_form = '
		<form name="validate_2fa_form" id="loginform" action="'. esc_url( self::login_url( array( 'action' => 'validate_2fa' ), 'login_post' ) ) .'" method="post" autocomplete="off">
				<input type="hidden" name="provider"      id="provider"      value="'. esc_attr( $provider ) .'" />
				<input type="hidden" name="wp-auth-id"    id="wp-auth-id"    value="'. esc_attr( $user->ID ) .'" />
				<input type="hidden" name="wp-auth-nonce" id="wp-auth-nonce" value="'. esc_attr( $login_nonce ) .'" />';

		if ( $interim_login ) {
			$custom_auth_form .= '<input type="hidden" name="interim-login" value="1" />';
		} else {
			$custom_auth_form .= '<input type="hidden" name="redirect_to" value="'. esc_attr( $redirect_to ) .'" />';
		}
		
		$custom_auth_form .= '<input type="hidden" name="rememberme" id="rememberme" value="'. esc_attr( $rememberme ) .'" />';
				
		$custom_auth_form .= '
			<fieldset>
				<div class="form-group tfa-input-row tfa-input-totp">
					<label for="authcode">Authorization Code</label>
					<input type="tel" name="authcode" id="authcode" class="form-control" value="" size="20" pattern="[0-9]*" maxlength="8" tabindex="1" autofocus required />
					<a href="#" class="toggle-tfa-input" data-input="backup_codes"><small>Or, use a backup code.</small></a>
				</div>
				<div class="form-group tfa-input-row tfa-input-backup_codes" style="display: none;">
					<label for="backupcode">Backup Code</label>
					<input type="tel" name="wp-2fa-backup-code" id="backupcode" class="form-control" value="" size="20" pattern="[0-9]*" maxlength="8" tabindex="1" required disabled />
					<a href="#" class="toggle-tfa-input" data-input="totp"><small>Use Google Authenticator authorization code.</small></a>
				</div>
				<div class="form-group-btn">
					<button type="submit" name="wp-submit" id="wp-submit" class="btn-sd btn-large" tabindex="2">Authenticate</button>
				</div>
			</fieldset>
		</form>';

		//save form in a transient
		set_transient( 'custom_2fa_auth_form_user_'. md5($user->user_login), $custom_auth_form, 300 );
	...
	public static function login_form_validate_2fa() {
		...
		if ( 'totp' === $provider && true !== self::validate_totp_authentication( $user ) ) {
			do_action( 'wp_login_failed', $user->user_login, 'totp' );		/*custom code - pass extra argument to get better control of error context */
		...
		if ( 'backup_codes' === $provider && true !== self::validate_backup_codes( $user ) ) {
			do_action( 'wp_login_failed', $user->user_login, 'backup_codes' );		/*custom code - pass extra argument to get better control of error context */
			$login_nonce = self::create_login_nonce( $user->ID );
			if ( ! $login_nonce ) {
				wp_die( esc_html__( 'Failed to create a login nonce.', 'wp-2fa' ) );
			}
		...
		if ( 'email' === $provider && true !== self::validate_email_authentication( $user ) ) {
			do_action( 'wp_login_failed', $user->user_login, 'email' );		/*custom code - pass extra argument to get better control of error context */
